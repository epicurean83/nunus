...
`play` 분기에 옛 두 줄을 손으로 되살리는 것으로는 **결함이 재현되지 않는다**. 같은 함수 끝의
`syncInputLock()`이 곧바로 다시 계산해 잠그기 때문이다. 실제 결함 코드로 돌려야 한다.

```bash
git checkout ac62749 -- public/index.html    # syncInputLock이 들어오기 전 상태
grep -c syncInputLock public/index.html      # 0이어야 한다 — 결함이 실재하는지 확인
```
...
참고: `test/multi-race.mjs`에는 이 작업과 무관한 기존 플레이크가 있다(앞선 레이스 시나리오에서
이을 낱말을 못 찾아 중단). 발생하면 다시 실행하되 몇 번 만에 성공했는지 보고에 남긴다.

- [ ] **Step 4: 고친 파일로 되돌려 통과 확인 (pass-after)**

```bash
git checkout 4fccda9 -- public/index.html
git diff public/index.html
```
Expected: `git diff` 출력 없음(Task 2 커밋과 동일)
