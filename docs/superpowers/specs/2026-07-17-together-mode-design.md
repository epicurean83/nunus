...
CORE 밖 재사용: `WORD_ALL` / `WORD_SET`(words.v1.txt 로드분 포함), `chosungIndex()` → `{map, easy}`(초성 20개↑ 패턴 풀). 위키 폴백은 혼자하기 전용이며 같이하기는 쓰지 않는다 — 아래 승부 판정 참조.
...
```
1. 로컬 검증 (CORE + WORD_SET, 위키 폴백 없음)  — 실패면 전송 안 함, 재시도 피드백
...
**왜 위키 폴백이 없는가**: 혼자하기의 `checkAnswer`는 로컬 사전에 없으면 위키낱말사전/위키백과에 네트워크로 물어보고 통과시켜준다. 같이하기에서 그걸 그대로 가져오면 제출 전에 네트워크 왕복을 기다려야 하는데, 레이스에서 그 시간은 곧 라운드를 남에게 내주는 시간이다 — 그래서 `checkMultiAnswer`는 동기적으로 로컬 사전(`CORE` + `WORD_SET`)만 보고 끝낸다. 대가는 솔직히 인정한다: 혼자하기라면 위키 폴백으로 통과했을 낱말이 같이하기에서는 사전에 없다는 이유로 거부될 수 있다.
...
타이머 워치독은 없다. **방은 사람의 액션으로만 움직인다** — 이건 빠뜨린 게 아니라 정한 것이다:

- 아무도 안 맞히고 아무도 교체 요청도 안 하면 → 그 라운드에 **그대로 멈춰 있는다**. 재촉하지 않는다.
- 모두 나가면 → `meta`가 그 상태로 남는다. 아무 일도 안 일어난다.
- 나중에 누가 들어와서 **답을 내거나 교체를 요청해야** 비로소 다시 굴러간다. 들어온 사람은 멈춰 있던 그 라운드·그 문제를 그대로 이어받는다(호스트는 자동 승계).

즉 "혼자 남았는데 방이 저절로 진도를 나가 있더라"가 없다. 시간이 아니라 사람이 게임을 민다.
...
| 로컬 검증 실패 | 기존 혼자하기와 같은 문구로 재시도 (게임오버 아님) — 같이하기는 위키 폴백 없이 로컬 사전만 본다 |
...
부수 사항: `words.v1.txt`(~33k, 검증용)는 비동기로 불러오고 실패는 조용히 삼킨다. 그 요청이 실패한 클라이언트는 내장 사전(`WORD_DICT`, ~4k)만으로 `WORD_SET`을 구성하게 되는데, 호스트가 초성게임 패턴을 뽑는 `chosungIndex().easy`는 33k쪽 기준이다 — 즉 그 클라이언트에게는 자신의 사전엔 없는 패턴이 출제될 수 있고, 그 세션은 사실상 그 사람에게 플레이가 안 되는 상태가 된다(진단 메시지 없음). 드물게만 일어나며, 고치지 않고 문서화만 한다.
...
DB 정리: 테스트로 생긴 데이터는 증분 규칙 때문에 클라이언트가 못 지우므로 관리자 권한으로 정리한 뒤 배포한다.

**배포 전에만** `/nunus` 통째 삭제가 안전하다 — 지금 이 RTDB에는 우리 테스트 데이터밖에 없다(인스턴스를 이번 작업에서 새로 만들었고, 공유 프로젝트의 NearBy 프로덕션은 이 RTDB를 쓰지 않는다 — 루트가 `null`). **배포 후에는 금지.** 그때부터 `/nunus`에는 남들이 실제로 하고 있는 게임이 들어있으므로, 지워야 하면 내가 만든 경로만 좁혀서 지운다.
