...
=== 같이하기 자동 게임 종료 제거 (2026-07-23, feature/remove-multi-dead-end) ===
- 사용자 지적(형평성): 호스트의 사전으로 자동 종료를 판정하니, 4k 사전 호스트가 남들은 이을 수
  있는 문제에서 방 전체를 끝냈다(리뷰 실측 ~30%). -> 같이하기 자동 종료를 제거.
- chainRoundOutcome 삭제(유일 호출자였던 hostTick에서 제거). 로직 테스트 85 -> 79.
- hostTick 라운드 전환을 round>=MULTI_ROUNDS 판정으로 되돌림. capturedMode는 유지(호스트 이탈 시
  null-deref 막는 별개 수정, chosung 분기가 씀).
- renderMulti 종료 문구 '게임 끝!'으로 되돌림.
- 막다름의 탈출구는 기존 교체 투표(참가자가 함께 결정 = 형평). 리뷰 확인: idle에서 교체 버튼
  항상 도달 가능, 요청자 자동 찬성 + 20초 묵시적 동의로 1클릭이면 방이 풀림. 쿨다운도 자가 복구.
- 혼자하기 막다른 종료는 유지(1명, 형평성 무관).
- 통합 3.8을 반전: "막다르러도 자동 종료 안 함 + 교체 버튼 있음" 검증. fail-before(자동 종료를
  인라인 재도입)로 4개 단언 FAIL 확인 후 제거. pass-after 전항목 PASS, exit 0.
- opus 리뷰: SHIP, findings 없음.
- 후속: 근본 원인인 words.v1.txt 로드 실패는 여전히 남은 이슈(원장 상단 참조).
