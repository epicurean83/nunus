// 같이하기(멀티) 통합 검증 — 실 브라우저 2개, 실 RTDB.
//
// 실행법:
...
//
// 이 스크립트는:
//   - puppeteer로 완전히 분리된 브라우저 컨텍스트(별도 프로필) 두 개를 띄워 실제
//     nearby-58e2d 프로젝트의 실 RTDB(nunus/chain)에 붙는다. 모킹 없음.
//   - 시작 전 nunus/chain, nunus/chosung 이 비어 있는지 확인하고(다른 진짜 플레이어가
//     쓰고 있을 수 있는 영구 방이므로), 끝나면 자신이 쓴 데이터만 지운다.
//   - 소요 시간: 대략 1.5~3분 (레이스 5회 + 교체투표 20초 대기 + 재연결 + 승계 포함).
...
//   (실측 결과 disconnect 핸들러가 UI를 잠그자마자 다른 렌더 호출이 거의 즉시 그걸
//   덮어써서 버튼이 안 사라진 것처럼 보이는 경우가 실제로 있었다 — 폴링으로는 놓치기
//   쉽다). 그래서 #multi-swap의 innerHTML 세터 자체를 가로채 모든 대입을 타임스탬프와
//   함께 기록하고, 오프라인 동안 빈 문자열로의 SET이 최소 한 번 실제로 일어났는지를
//   증거로 확인한 다음에만 "재연결 후 버튼이 클릭 가능하다"를 검사한다.

import puppeteer from 'puppeteer';

const URL_ = 'http://localhost:8777/';
const PROJECT = 'nearby-58e2d';
const MODE = 'chain';                 // 끝말잇기 방만 사용 (초성은 별도 영구 방, 안 건드림)
const ROOM = 'nunus/' + MODE;
const MULTI_ROUNDS = 10, REVEAL_MS = 3000, SWAP_VOTE_MS = 20000;
...
async function readScores(page, gameId){
  return page.evaluate((gid) => new Promise(res => {
    firebase.database().ref('nunus/chain/scores/' + gid).once('value', s => res(s.val() || {}));
  }), gameId);
}
  ...
  await page.evaluate(() => {
    ...
    proto.transaction = function(updateFn, onComplete, applyLocally){
      ...
      const wrapped = (err, committed, snap) => {
        window.__txnLog.push({ key, committed: !!committed, err: err ? String(err) : null, t: Date.now() });
...
const metaTxnLog = page => page.evaluate(() => window.__txnLog.filter(e => e.key === 'meta'));
...
async function installSwapWatch(page){
  await page.evaluate(() => {
    ...
    Object.defineProperty(el, 'innerHTML', {
      ...
      set(v){ window.__swapSetLog.push({ t: Date.now(), html: v }); return desc.set.call(this, v); }
    });
  });
}
...
let A = null, B = null;   // { page, ctx, nick }
...
try{
  ...
  A = await join(browser, '가가');
  B = await join(browser, '나나');
  ...
  /* 2) GAP 1 — 진짜 경합하는 동시 제출을 5번, 재시도 포함 */
  const raceResults = [];
  let attempts = 0;
  ...
  while (raceResults.length < 5 && attempts < MAX_ATTEMPTS){
    ...
    const scoresBefore = await readScores(A.page, gameId);
    const roundBefore = await round(A.page);
    await clearTxnLog(A.page); await clearTxnLog(B.page);

    const t = Date.now() + 1200;
    await Promise.all([A.page, B.page].map(p => p.evaluate((t, w) => {
      document.getElementById('multi-input').value = w;
      setTimeout(() => submitMulti(), Math.max(0, t - Date.now()));
    }, t, word)));
    ...
    await sleep(600);
    const scoresAfter = await readScores(A.page, gameId);
    ...
    // 3초 후 라운드 진행 확인 (첫 유효 레이스에서만 타이밍까지 체크)
    ...
    if (!timedFirstAdvance){
      ...
      const withinWindow = elapsed >= 2000 && elapsed <= 6500;
      report('reveal 후 약 3초 뒤 양쪽 다 다음 라운드로 진행', withinWindow && roundAfterA === roundAfterB,
        '경과=' + elapsed + 'ms 이전라운드=' + roundBefore + ' A=' + roundAfterA + ' B=' + roundAfterB);
    } else {
        ...
        문제만 바뀌는지도 함께 확인한다(끝말잇기 교체 재출제). */
  {
    ...
    // 오프라인 핸들러가 실제로 UI를 잠그려고 했는지(빈 문자열로 SET)를 증거로 확인.
    // 이게 없으면 "버튼이 돌아왔다"는 애초에 안 없어졌던 것과 구분이 안 된다.
  ...
  /* 4) 호스트 승계 — 호스트가 탭을 닫아도 남은 쪽이 진행 */
  {
    ...
    await cleanupOwn(hostSide.page, gameIdsUsed);
  ...
  // 이번 실행에서 만든 것만 정확히 지운다: 우리 두 uid의 presence/점수, 그리고 meta
  // 전체(메타는 규칙상 인증된 아무나 쓸 수 있어 마지막에 지워도 안전 — 단 preflight로
  // 시작 전 비어있었음을 이미 확인했으므로 남아있는 meta는 전부 우리가 만든 것뿐이다).
  try{
    const survivor = A || B;
    if (survivor){
      await cleanupOwn(survivor.page, gameIdsUsed);
      await survivor.page.evaluate(() => firebase.database().ref('nunus/chain/meta').remove());
    }
...
async function cleanupOwn(page, gameIds){
  await page.evaluate((gids) => {
    const uid = MG.uid;
    const ops = [firebase.database().ref('nunus/chain/presence/' + uid).remove()];
    for (const gid of gids){
      ops.push(firebase.database().ref('nunus/chain/scores/' + gid + '/' + uid).remove());
    }
